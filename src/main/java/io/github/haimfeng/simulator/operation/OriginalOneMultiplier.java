...
public class OriginalOneMultiplier implements ArithmeticOperation {
    ...
    public List<StepRecord> execute(Operand op1, Operand op2) {
        ...
        steps.add(new StepRecord("INIT", a + " " + q, String.valueOf(q.charAt(q.length() - 1)),
                "$\\text{Init: } A=0,\\; Q=|Y|$"));

        for (int i = 1; i <= m; i++) {
            char qLast = q.charAt(q.length() - 1);
            String operation;

            if (qLast == '1') {
                a = NumberUtils.addBinary(NumberUtils.padLeftZero(a, m), absX);
                if (a.length() > m) {
                    a = a.substring(a.length() - m);
                }
            }

            String combined = a + q;
            combined = "0" + combined.substring(0, combined.length() - 1);
            a = combined.substring(0, m);
            q = combined.substring(m);

            String stepDesc = qLast == '1'
                    ? "$A = A + |X|;\\; \\texttt{>>>}1$"
                    : "$\\texttt{>>>}1$";
            steps.add(new StepRecord("Step " + i, a + " " + q, String.valueOf(q.charAt(q.length() - 1)), stepDesc));
        }
    ...
    public String getOperationName() {
        return "Original-Code 1-Bit Multiplication";
    }

    @Override
    public String[] getColumnHeaders() {
        return new String[]{"Step", "A Q", "Q₀", "Info."};
    }
